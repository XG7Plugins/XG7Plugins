...
public class NPC1_8_1_16 extends NPC {
    ...
    private static final ReflectionClass enumItemSlotClass = NMSUtil.getNMSClass("EnumItemSlot");
        ...
        Bukkit.getScheduler().runTask(XG7Plugins.getInstance(), () -> {
            try {
                ...
                ReflectionObject npc = NMSUtil.getNMSClass("EntityPlayer")
                        ...
                        .newInstance(playerNMS.getCraftPlayerHandle().getField("server"), nmsWorld.getObject(), skin,interactManager.getObject());
                ...
                dataWatcher.watch(XG7Plugins.getMinecraftVersion() > 13 ? 17 : XG7Plugins.getMinecraftVersion() < 9 ? 10 : 16, XG7Plugins.getMinecraftVersion() > 8 ? (byte) 0x07F : (byte) 0xFF);
                    ...
                    if (!equipments.isEmpty()) {
                        if (XG7Plugins.getMinecraftVersion() == 16) {
                            ReflectionObject packetPlayOutEntityEquipment = NMSUtil.getNMSClass("PacketPlayOutEntityEquipment")
                                    ...
                                    .newInstance(npc.getMethod("getId").invoke(), Pair.toMojangPairList((List<Pair<?, ?>>) equipments));
                            playerNMS.sendPacket(packetPlayOutEntityEquipment.getObject());
                        } else {
                            for (Pair<?, ?> pair : (List<Pair<?, ?>>) equipments) {
                                ReflectionObject packetPlayOutEntityEquipment = NMSUtil.getNMSClass("PacketPlayOutEntityEquipment")
