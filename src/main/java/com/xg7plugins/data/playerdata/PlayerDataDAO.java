...
public class PlayerDataDAO implements DAO<UUID, PlayerData> {
    ...
    public CompletableFuture<List<PlayerData>> getAll() {
        return CompletableFuture.supplyAsync(() -> {
            try {
                return Query.selectAllFrom(getPlugin(), PlayerData.class).waitForResult().getList(PlayerData.class);
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
        });
    ...
    @Override
    public CompletableFuture<Boolean> delete(PlayerData data) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                Transaction.delete(XG7Plugins.getInstance(), data).onError(e -> {
                    throw new RuntimeException(e);
                }).waitForResult();
                XG7PluginsAPI.database().unCacheEntity(getPlugin(), data.getPlayerUUID().toString());
                return true;
            } catch (IllegalAccessException | InvocationTargetException | NoSuchMethodException |
                     InstantiationException e) {
                throw new RuntimeException(e);
